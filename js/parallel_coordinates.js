	...
	$.get('data/somatic.graphite.vcf', function (vcfString) {
		...
		for (var sampleName in samples) {
			...
			for (var i = 0; i < sample.variants.length; ++i) {
				var sampleData = {};
				sampleData[sampleName] = sample.variants[i].getAlternateAlleleFrequency();
				if (isNaN(sample.variants[i].getAlternateAlleleFrequency())) { console.log('isNaN'); }
				if (firstTimeThrough) {
					formattedVariants.push(sampleData);
				}
				else {
					formattedVariants[i][sampleName] = sample.variants[i].getAlternateAlleleFrequency();
				}
			}
			firstTimeThrough = false;
		}
		console.log(formattedVariants);
		createParallelCoordinatesGraph(formattedVariants);
	});
}
...
function createParallelCoordinatesGraph(data) {
	...
	x.domain(dimensions = d3.keys(data).filter(function(d) {
		y[d] = d3.scale.linear().domain([1, 0]).range([height, 0]);
		return true;
	}));
